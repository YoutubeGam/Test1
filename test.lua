local message=loadstring(game:HttpGet('https://pastebin.com/raw/si4m9rWz',true))();
if game.PlaceId==301549746 and game:GetService'Players'.LocalPlayer.UserId==2729779293 and message.ActivedToday then
...
local function notice(Text)
    ...
    rconsoleprint('[')
    rconsoleprint('@@GREEN@@')
    rconsoleprint('NOTICE')
    rconsoleprint('@@WHITE@@')
    rconsoleprint('] ')
    rconsoleprint(Text..'\n')
end;
local function sussec(Text)
    rconsoleprint('@@WHITE@@')
    rconsoleprint('[')
    rconsoleprint('@@GREEN@@')
...
local max=math.random(1,10)
notice('Esperar '..max..'s estaremos buscandos dados no Servidor')
wait(max)
local max=math.random(1,10)
notice('Conectando a HTTPS')
wait(max)
rconsoleerr('Estamos tentando resolver')
wait(max)
sussec('Voce esta conectado a nossa Rede, Bem-vindo, todas as suas informacao estao sendo Criptografado')
wait(3)
printc('\nApartir da linha de baixo serao dados, nao ligue por isto, apenas ignore.')
wait(7)
for i=1, math.random(1,1842) do
    printc(RandomVariable(math.random(25,30)))
    wait()
end;
wait(1)
rconsoleclear();
notice('Pronto aproveite o script!');
wait(3)
rconsoleclear();
printc('Usuario: '.. game:GetService('Players').LocalPlayer.Name..' - Sua conta e Demo, aproveite ter este cargo, qualquer momento posso ser Demoted')
...
local cfglocation = "YTDevscfg/"
makefolder("YTDevscfg")
...
function library:New(name)
	...
	local YTDevs = Instance.new("ScreenGui")
	...
	YTDevs.Name = "YTDevs"
	YTDevs.ResetOnSpawn = false
	YTDevs.ZIndexBehavior = "Global"
	YTDevs.DisplayOrder = 420133769

	local UIScale = Instance.new("UIScale")
	UIScale.Parent = YTDevs
	...
	game:GetService("UserInputService").InputBegan:Connect(function(key)
		if key.KeyCode == Enum.KeyCode.RightShift then
			YTDevs.Enabled = not YTDevs.Enabled
			library.uiopen = YTDevs.Enabled
		end
	end)
...
makefolder("YTDevslua")

local allluas = {}

for _,lua in pairs(listfiles("YTDevslua")) do
	local luaname = string.gsub(lua, "YTDevslua\\", "")
	table.insert(allluas, luaname)
end

RunService.RenderStepped:Wait()

local gui = library:New("YTDevs")
...
luascripts:Element("Button", "load", {}, function()
	loadstring(readfile("YTDevslua\\"..values.luas["lua scripts"].lua.Scroll))()
end)
...
chat:Element("Toggle", "chat spam", nil, function(tbl)
	if tbl.Toggle then
		while values.misc.chat["chat spam"].Toggle do
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "hi" or "buy YTDevs.me", false, "Innocent", false, true)
			wait(values.misc.chat["speed (ms)"].Slider/1000)
		end
	end
end)
chat:Element("Dropdown", "type", {options = {"standard", "YTDevs"}})
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)
	...
	if values.misc.chat["kill say"].Toggle then
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "looks like you don't have YTDevs", false, "Innocent", false, true)
	end
end)
